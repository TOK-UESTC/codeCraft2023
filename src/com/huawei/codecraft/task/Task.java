...
public class Task {
    ...
    private Map<Integer, List<Node>> graph; // 数据结构图, key: 工作台类型， value: 以该工作台发出的任务node列表
    private Map<Integer, List<Node>> workbenchNodeMap; // key: 工作台ID，value: 该工作台发布的任务

    // 初始化地图建模，按照自己的想法实现地图数据模型
    public Task(Map<Integer, List<Workbench>> workbenchTypeMap) {
        this.workbenchTypeMap = workbenchTypeMap;
        this.graph = new HashMap<Integer, List<Node>>();
        this.workbenchNodeMap = new HashMap<Integer, List<Node>>();
        buildGraph();
    }
...
/**
 * @description: 节点类封装了任务，一个任务就是一个节点，包含将什么产品从源工作台运送到目的工作台的信息
 */

class Node {

    private Workbench from; // 任务来源工作台
    private Workbench to; // 任务目的工作台
    private int productType; // 物品型号
    private double priority; // 优先级
    private double award; // 完成任务预计奖励，用预计利润
    private double distance; // 任务距离
    private boolean visited; // 是否被访问
    private List<Node> postTaskList; // 后继任务列表

    public Node(Workbench from, Workbench to, int productType, double priority, double award, double distance) {
        this.from = from;
        this.to = to;
    ...
    public void setFrom(Workbench from) {
        this.from = from;
    }
    ...
    public void setTo(Workbench to) {
        this.to = to;
    }
